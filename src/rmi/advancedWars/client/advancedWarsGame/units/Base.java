...
public class Base {
	...
	public boolean attack(int destx, int desty, boolean returnfire) {
		...
		Base target = FindTarget(destx, desty, true, false);
		if (target!=null) {
	...
	private void capture(int destx,int desty) {
		...
		if (destx==x&&desty==y) {
			if (bld!=-1) {
				rmi.advancedWars.client.advancedWarsGame.buildings.Base city = Game.builds.get(bld);
	...
	private Base FindTarget(int destx, int desty, boolean hostilefire, boolean friendlyfire) {
