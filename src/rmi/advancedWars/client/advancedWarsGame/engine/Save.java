...
public class Save {
	...
	public void LoadSettings() {
		...
		catch (Exception e) {Game.error.ShowError("Player data failed to load." + System.getProperty("user.dir")); e.printStackTrace();}
	}
	...
	private String SaveCityData() {
		...
		for (int i = 0; i < Game.builds.size(); i++) {
	    	rmi.advancedWars.client.advancedWarsGame.buildings.Base bld = Game.builds.get(i);
	...
	private String SaveUnitData() {
	    ...
	    for (int i = 0; i < Game.units.size(); i++) {
	    	rmi.advancedWars.client.advancedWarsGame.units.Base unit = Game.units.get(i);
