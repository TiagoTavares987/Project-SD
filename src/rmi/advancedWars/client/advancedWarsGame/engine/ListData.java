...
import rmi.advancedWars.client.advancedWarsGame.buildings.*;
import rmi.advancedWars.client.advancedWarsGame.players.*;
import rmi.advancedWars.client.advancedWarsGame.players.Base;
import rmi.advancedWars.client.advancedWarsGame.units.*;
...
public class ListData {
	 ...
	 */
	public Base CreateCommander(int which, int team, int money, boolean npc) {
		switch (which) {
	 ...
	 */
	public rmi.advancedWars.client.advancedWarsGame.units.Base CreateUnit(int type, int owner, int xx, int yy, boolean active) {
		switch (type) {
	...
	public void LoadCommander(String Name, boolean Defeated, int Team, int Money, int Kills, int Loses, int Power, int Using, boolean npc) {
		for (int i = 0; i < Game.displayC.size(); i++) {
			if (Game.displayC.get(i).name.equals(Name)) {
				...
				Base ply = Game.player.get(Game.player.size()-1);
	...
	public void LoadCity(String Name, int Owner, int Health, int id) {
		...
		rmi.advancedWars.client.advancedWarsGame.buildings.Base bld = Game.builds.get(id);
		bld.owner = Owner;
		bld.health = Health;
	}
	...
	public void LoadUnit(String Name, int Owner, int Health, int Ammo, int Fuel, int X, int Y, boolean Acted) {
		for (int i = 0; i < Game.displayU.size(); i++) {
			if (Game.displayU.get(i).name.equals(Name)) {
				...
				rmi.advancedWars.client.advancedWarsGame.units.Base unit = Game.units.get(Game.units.size()-1);
				unit.health = Health;
				unit.Ammo = Ammo;
				unit.Fuel = Fuel;
			}
