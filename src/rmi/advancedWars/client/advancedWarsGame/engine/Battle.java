...
import rmi.advancedWars.client.advancedWarsGame.menus.EndBattle;
...
public class Battle {
	...
	public void NewGame(String mapname) {
		Game.player = new ArrayList<Base>();
		Game.builds = new ArrayList<rmi.advancedWars.client.advancedWarsGame.buildings.Base>();
		Game.units = new ArrayList<rmi.advancedWars.client.advancedWarsGame.units.Base>();
		Game.view.Loc.x = 0;
		Game.view.Loc.y = 0;
	...
	public void EndTurn() {
		Base ply = Game.player.get(currentplayer);
		for (rmi.advancedWars.client.advancedWarsGame.units.Base unit : Game.units) {
			unit.acted=false;
			unit.moved=false;
		}
		currentplayer++;
	...
	public void Buyunit(int type, int x, int y) {
		try {
			if(Game.btl.currentplayer != Game.observer.getId())
				return;
			Game.observer.getSubjectRI().setState(new State(String.valueOf(Game.observer.getId()), "b " + type + " " + x + " " + y));
		} catch (RemoteException e) {
			e.printStackTrace();
		}
	}

	public void RemoteBuyUnit(int type, int x, int y) {
