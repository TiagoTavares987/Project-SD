...
public class Map {
	...
	public rabbitmq.advancedWars.client.game.terrain.Base[][] map;
	public List<rabbitmq.advancedWars.client.game.terrain.Base> tiles = new ArrayList<rabbitmq.advancedWars.client.game.terrain.Base>();
	public MapParser parse = new MapParser();
	
	public Map() {
		LoadTiles();
		map = new rabbitmq.advancedWars.client.game.terrain.Base[height][width];
	}
	...
	public void MapSetup(int width, int height) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				map[y][x] = new rabbitmq.advancedWars.client.game.terrain.Dirt();
			}
		}
	}
	...
	private void ChangeTiles(rabbitmq.advancedWars.client.game.terrain.Base type, int x, int y){
	...
	public rabbitmq.advancedWars.client.game.terrain.Base getTile(int i) {
		switch(i) {
		case 0:return new rabbitmq.advancedWars.client.game.terrain.Dirt();
		case 1:return new rabbitmq.advancedWars.client.game.terrain.Forest();
		case 2:return new rabbitmq.advancedWars.client.game.terrain.Mountain();
		case 3:return new rabbitmq.advancedWars.client.game.terrain.Water();
		case 4:return new rabbitmq.advancedWars.client.game.terrain.City();
		case 5:return new rabbitmq.advancedWars.client.game.terrain.Road();
		default:return null;
		}
	}
	...
	public void ResizeMap(int neww, int newh, int oldw, int oldh) {
		...
		width = neww;
		height = newh;
		rabbitmq.advancedWars.client.game.terrain.Base[][] newmap = new rabbitmq.advancedWars.client.game.terrain.Base[height][width];
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {//TODO: Fix this as it seems to not be exactly correct.
				if (oldh < y || oldw < x) {
					...
					newmap[y][x] = new rabbitmq.advancedWars.client.game.terrain.Dirt();
				}
			}
		}
		map = newmap;
