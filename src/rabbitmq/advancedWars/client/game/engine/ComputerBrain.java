...
import rabbitmq.advancedWars.client.game.units.Base;
...
public class ComputerBrain {
	...
	public void ThinkDamnYou(rabbitmq.advancedWars.client.game.players.Base ply) {
		...
		if (!DoneUnits) {
			rabbitmq.advancedWars.client.game.units.Base unit = Game.units.get(UnitCount);
			UnitCount++;
			...
			rabbitmq.advancedWars.client.game.buildings.Base bld = Game.builds.get(CityCount);
			CityCount++;
	...
	private void HandleBuilding(rabbitmq.advancedWars.client.game.buildings.Base bld) {
		if (!bld.Menu.equals("")) {
			if (!bld.Locked) {
	...
	/**Looks for any enemies in range at the edu.ufp.inf.sd.rmi._00_project.client.game.units current location by attacking each location in the attackable locations.*/
	private boolean WannaHug(rabbitmq.advancedWars.client.game.units.Base unit) {
	...
	private boolean BuildingInRange(rabbitmq.advancedWars.client.game.units.Base unit) {
