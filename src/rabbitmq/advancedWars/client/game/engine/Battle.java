...
public class Battle {
	/**A count of all the edu.ufp.inf.sd.rmi._00_project.client.game.players in the game, used before Game.player is populated so this is required.*/
	...
	public void NewGame(String mapname) {
		Game.player = new ArrayList<rabbitmq.advancedWars.client.game.players.Base>();
		Game.builds = new ArrayList<rabbitmq.advancedWars.client.game.buildings.Base>();
	...
	public void EndTurn() {
		rabbitmq.advancedWars.client.game.players.Base ply = Game.player.get(currentplayer);
		for (rabbitmq.advancedWars.client.game.units.Base unit : Game.units) {
			unit.acted=false;
			unit.moved=false;
		}
		currentplayer++;
	...
	/**Grabs the number of edu.ufp.inf.sd.rmi._00_project.client.game.buildings a player owns.*/
	...
	public void MaxUsers(int max) {
		//Setup for max edu.ufp.inf.sd.rmi._00_project.client.game.players
		if (max<2) {totalplayers = 2;}
		else if (max>12) {totalplayers = 12;}
		else {totalplayers = max;}
		//HACK: Change when I add more images to support more edu.ufp.inf.sd.rmi._00_project.client.game.players.
		if (max>4) {totalplayers = 4;
			Game.error.ShowError("The game currently supports only 4 edu.ufp.inf.sd.rmi._00_project.client.game.players, not " + max + ".");
		}
	}
	...
	public void CaptureCapital(int x, int y) {
		...
		for (int i = 0; i < Game.builds.size(); i++) {
			if (Game.builds.get(i).x == x && Game.builds.get(i).y == y) {
				...
				if (playersleft<=1) {
					GameOver = true;
					new rabbitmq.advancedWars.client.game.menus.EndBattle();
				}
				break;
			}
		}
		...
		//TODO: Change all edu.ufp.inf.sd.rmi._00_project.client.game.buildings to be owned by the player.
	}
